...
各詩行ベクトル `x_i` と参照ベクトル `ref_i` のコサイン類似度に基づき、  
次のように定義します：
...
参照ベクトル `ref_i` は読者体験をモデル化し、逐次ローカル基準（±3行の平均）を採用しています。

> **コサイン類似度とは？**  
> 二つのベクトルが成す「角度」の近さを測る尺度。  
> 値域は -1 ～ +1。+1 なら完全に同じ方向、0 なら直交、-1 なら正反対。  
> 角度が開けば「隔たり」が生じ、そこにこそ新しい意味や詩が生まれる──  
> Divergence は、その隔たりを「跳躍の強度」として受けとめ直したものです。
